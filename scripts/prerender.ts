...
/**
 * Per-slug SEO override for product pages lives in
 * `data/productMetaOverride.ts` and is imported above. Single source of
 * truth — shared with `pages/ShopProductDetail.tsx` so SPA route changes
 * also render Vera FINAL meta (not the Shopify-synced fallback copy).
 *
 * To update: edit `data/productMetaOverride.ts`. Do NOT edit
 * `data/shop-products-fallback.json` — those edits are erased on the next
 * build by `scripts/sync-shop-fallback.ts`.
 */
