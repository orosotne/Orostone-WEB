...
export const ShopProductDetail: React.FC = () => {
  ...
  // Prefer Vera FINAL meta (PRODUCT_META_OVERRIDE — single source of truth,
  // shared with scripts/prerender.ts) over Shopify-synced fallback data.
  // The Shopify metaTitle/metaDescription fields are regenerated by
  // sync-shop-fallback.ts on every build, so without this override the
  // SPA route-change render would drift back to old Shopify copy even when
  // the prerendered HTML is correct.
